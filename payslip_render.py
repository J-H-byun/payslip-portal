...
def build_payslip_card_html(member_key: str, data: dict, pay_year: int, pay_month: int, pay_date_str: str = None) -> str:
    ...
    _guk_net = (
        data["국비_기본급"] + data["국비_주휴수당"] + data["국비_할증"] + data["국비_연차수당"]
    )
    _do_net = (
        data["도비_기본급"] + edu_allowance + data["도비_주휴수당"] + data["도비_할증"] + data["도비_연차수당"]
    )
    _si_net = (
        data["시비_기본급"] + data["시비_주휴수당"] + data["시비_할증"] + data["시비_연차수당"]
    )
    # 재원별 실지급액(위 표) = 원본 시트값 대신, 표 안의 위 항목들(기본급[교육포함]+주휴+할증+연차)을 직접 합산.
    # (원본 실지급액 값을 그대로 쓰면, 도비 근무시간이 0이면서 교육수당만 있는 달에 실지급액이 0으로
    #  잘못 나오는 사례가 있어, 표 안에서 항상 앞뒤가 맞도록 직접 합산 방식으로 계산함)
