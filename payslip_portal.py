...
def load_snapshot():
    ...
    for row in rows:
        if len(row) < 2 or not row[0].strip():
            continue
        try:
            data[row[0].strip()] = json.loads(row[1])
    ...
    return data, pay_year, pay_month, updated_at
...
if st.session_state["authed_member"] is None:
    ...
    if submitted:
        ...
        if not name_clean or len(birth_clean) != 6:
            ...
            login_key = f"{name_clean}{birth_clean}"
            locked, remain_sec = _check_lock(login_key)
            ...
            if locked:
                ...
                try:
                    data, pay_year, pay_month, updated_at = load_snapshot()
                ...
                if login_key in data:
                    ...
                    st.session_state["authed_member"] = login_key
                    st.rerun()
                else:
                    _register_fail(login_key)
